...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderCart = () => {
        ...
        cart.forEach((item, index) => {
            const row = `
                    <tr>
                        <td><img class="img_deg" src="${item.image}" alt="${item.name}"></td>
                        <td>${item.name}</td>
    ...
    cartItemsContainer.addEventListener('input', (e) => {
        if (e.target.classList.contains('quantity-input')) {
            ...
            const avaStock = parseInt(e.target.dataset.stock)

            if (avaStock => newQuantity > 0) {
                ...
                alert('Quantity must be at least 1 and product must limited to ' + avaStock);
